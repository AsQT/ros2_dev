...
def generate_launch_description():
    ...
    description_parent  = os.path.dirname(robot_description)
    gazebo_share_parent = os.path.dirname(robot_gazebo)

    worlds_dir          = os.path.join(robot_gazebo, "worlds")
    world_file          = os.path.join(robot_gazebo, "worlds", "table", "arm_on_the_table.sdf")
    box_file            = os.path.join(robot_gazebo, "worlds", "box", "box_model.sdf")
    ...
    # 7) Spawn white box obstacle after robot
    spawn_box_node  = Node(
                package     ="robot_gazebo",
                executable  ="box_obstacle.py",
                output      ="screen",
                parameters  =[
                            {"table_z":     1.10},
                            {"box_height":  0.08},
    ...
    spawn_box_after_robot = RegisterEventHandler(
                                OnProcessExit(
                                    target_action=spawn_robot,
                                    on_exit=[TimerAction(period=1.5, actions=[spawn_box_node])], ) )

    spawn_box = [
        spawn_box_after_robot, ]
