"""YOLO + aligned depth debug node for camera-frame 3D coordinates.

This node is intentionally limited to camera coordinates. It does not publish
ObjectArray/ObjectDetection and does not transform detections into robot/base/world
frames because camera extrinsics are not available in this calibration step.
"""
...
def _default_model_path() -> str:
    return "/home/asus/ros_vision/src/robot_vision_pipeline/models/yolov8.pt"


class YoloDepthToCameraNode(Node):
    """Detect wood objects and estimate their 3D camera-frame coordinates."""

    def __init__(self) -> None:
        super().__init__("yolo_depth_to_camera")
        ...
        self.declare_parameter("debug_image_topic", "/calib/yolo_depth_debug_image")
        ...
        if not model_path or not os.path.isfile(model_path):
            self.get_logger().error(
                "Không tìm thấy YOLO model. Hãy truyền parameter model_path, ví dụ:\n"
                "  ros2 run robot_vision_pipeline yolo_depth_to_camera "
                "--ros-args -p model_path:=/duong/dan/toi/best.pt\n"
                f"Giá trị hiện tại: '{model_path}'"
            )
        ...
        self.get_logger().info("YoloDepthToCameraNode started")
        ...
        self.get_logger().info("No base/world transform is applied in this calib node.")
        self.get_logger().info("========================================")
    ...
    def _pixel_to_camera(self, u: int, v: int, z: float) -> Optional[tuple[float, float, float]]:
        ...
        cx = float(k[2])
        cy = float(k[5])
        if fx == 0.0 or fy == 0.0 or not all(math.isfinite(x) for x in (fx, fy, cx, cy)):
            self.get_logger().warning("CameraInfo K không hợp lệ, bỏ qua frame hiện tại.")
            return None
    ...
    def _on_color_image(self, msg: Image) -> None:
        ...
        if self.camera_info is None:
            if not self.warned_no_info:
                ...
                self.get_logger().warning("Chưa có CameraInfo, chưa tính tọa độ 3D.")
        ...
        if self.latest_depth is None:
            if not self.warned_no_depth:
                ...
                self.get_logger().warning("Chưa có aligned depth image, chưa tính tọa độ 3D.")
            self._publish_debug(debug, msg)
            return
        ...
        for result in results:
            ...
            for box in boxes:
                ...
                # Future step, intentionally disabled in this calib node:
                # - publish ObjectArray/ObjectDetection
                # - transform (X, Y, Z) from camera frame to robot/base/world frame
                #   using camera extrinsics or TF.
...
def main(args: list[str] | None = None) -> None:
    ...
    node: Optional[YoloDepthToCameraNode] = None
    try:
        node = YoloDepthToCameraNode()
        rclpy.spin(node)
