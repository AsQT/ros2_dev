#!/usr/bin/env python3
"""Adapt YOLO JSON detections into object detection messages by class.

Subscribes to YOLO JSON output and aligned depth image.
Publishes BoxDetection messages for the object classes used by the current
pipeline:
  /vision/wood_detection  -> class_name == "wood"
  /vision/box_detection   -> class_name == "box"

The message type is still BoxDetection for compatibility with the existing
message package, but this adapter node is named as a generic object adapter
because it is not only responsible for the box class.
"""
...
class YoloJsonToObjectDetectionNode(Node):
    def __init__(self) -> None:
        super().__init__("yolo_json_to_object_detection_node")
        ...
        self._pub_wood = self.create_publisher(
            BoxDetection, "/vision/wood_detection", det_qos
        )
        self._pub_box = self.create_publisher(
            BoxDetection, "/vision/box_detection", det_qos
        )

        self.create_subscription(String, detections_topic, self._on_yolo_json, det_qos)
        ...
        self.get_logger().info(
            "YOLO JSON object adapter started | "
    ...
    def _on_yolo_json(self, msg: String) -> None:
        ...
        if not isinstance(detections, list):
            self.get_logger().warn(
                "YOLO JSON field 'detections' is not a list; skipping payload."
            )
            return
        if not detections:
...
def main(args=None) -> None:
    ...
    node = YoloJsonToObjectDetectionNode()
    try:
        rclpy.spin(node)
