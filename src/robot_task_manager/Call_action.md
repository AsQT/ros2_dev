...
- `CheckerBoard` server thật tạo action tên `/move_checker_board`, nhưng `task_manager_client.cpp` lại tạo client tới `checker_board`. Khi gọi CLI nên dùng `/move_checker_board`.
...
| `/repeatability_test` | `robot_task_manager/action/RepeatabilityTest` | `repeatability_test_server` | `repeatability_test_action_server` | Test repeatability theo X/Y |
...
Default goal trong `task_manager_client`: `start=true`.
...
ros2 action send_goal /move_to_pose robot_task_manager/action/MoveToPose \
  "{target_pose: {position: {x: 0.40, y: 0.10, z: 0.35}, orientation: {x: 0.0, y: 0.0, z: 0.0, w: 1.0}}, velocity_scale: 0.5}" \
  --feedback
```
...
ros2 action send_goal /move_checker_board robot_task_manager/action/CheckerBoard \
  "{step: 0.10, velocity_scale: 0.5}" --feedback
```

`task_manager_client` có nhánh `task_name:=checker_board`, nhưng hiện client connect tới `checker_board` thay vì `/move_checker_board`, nên có thể không thấy server nếu chưa sửa code.
...
ros2 action send_goal /pickplace robot_task_manager/action/PickPlace \
  "{pose_pick: {position: {x: 0.40, y: 0.10, z: 0.25}, orientation: {x: 1.0, y: 1.0, z: 0.0, w: 0.0}}, pose_place: {position: {x: 0.30, y: 0.00, z: 0.25}, orientation: {x: 1.0, y: 1.0, z: 0.0, w: 0.0}}, gripper: 0.01, velocity_scale: 0.5}" \
  --feedback
```
...
ros2 action send_goal /drl_pickplace robot_task_manager/action/DrlPickPlace \
  "{target_pick: {header: {frame_id: 'base_link'}, pose: {position: {x: 0.40, y: 0.05, z: 0.08}, orientation: {x: 0.7071068, y: 0.7071068, z: 0.0, w: 0.0}}}, target_place: {header: {frame_id: 'base_link'}, pose: {position: {x: 0.34, y: -0.10, z: 0.08}, orientation: {x: 0.7071068, y: 0.7071068, z: 0.0, w: 0.0}}}, gripper_close_width_m: 0.028}" \
  --feedback
```
...
ros2 launch robot_task_manager drl_pick_place_random_test.launch.py \
  number_of_trials:=20 random_seed:=0 gripper_close_width_m:=0.028
```

Với Gazebo:

```bash
ros2 launch robot_task_manager drl_pick_place_random_test.launch.py use_gazebo:=true
```
...
- `repeat_count > 0`.
- `axis` chỉ nhận `0` (`AXIS_X`) hoặc `1` (`AXIS_Y`).
...
2. Mỗi loop:
   - Cartesian tới `meas_pose`.
   - Wait `measurement_settle_time_s`.
   - Cartesian quay về `retract_pose`.
   - MoveToPose tới `disturb_pose_1`.
   - MoveToPose tới `disturb_pose_2`.
   - MoveToPose quay về `retract_pose`.
...
ros2 launch robot_task_manager repeatability_test_client.launch.py \
  axis:=0 repeat_count:=3 meas_offset:=0.02 velocity_scale:=0.25 frame_id:=world
```
...
| `checker_board` | `checker_board` trong code, nhưng server đang là `/move_checker_board` |
