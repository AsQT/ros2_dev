...
class RvizEmbedder(QObject):
    ...
    def start(self) -> None:
        ...
        self._log(f"Started RViz pid={self.rviz_pid}")
        self._log(f"RViz process started pid={self.rviz_pid}")
    ...
    def resize_embedded_rviz(self) -> None:
        ...
        self._adopt_embedded_child_if_available()
    ...
    def find_rviz_window_by_pid(self, pid: int) -> Optional[str]:
        ...
        selected_area = -1
        content_candidates = [
            window for window in candidates if not bool(window.get("is_top_level", False))
        ]
        content_selected = self._largest_valid_content_candidate(pid, content_candidates)
        ...
        for window in candidates:
            ...
            self._log_once(
                ...
                f"Candidate window: id={window_id}, pid={pid}, title={title}, class={wm_class}, size={width}x{height}",
            )
        ...
        if content_selected is not None:
            ...
            self._log_once(
                f"wait-content:{pid}",
                f"Waiting for RViz content window for pid={pid}; top-level window is not embedded directly",
            )
            return None
    ...
    def _embed_window(self) -> None:
        ...
        embedded_child = self._largest_rviz_descendant(container_win_id)
        if embedded_child and embedded_child != self.rviz_window_id:
        ...
        self._run_xdotool(["windowmap", self.rviz_window_id])
        if self.rviz_top_level_window_id and self.rviz_top_level_window_id != self.rviz_window_id:
            self._park_top_level_window(self.rviz_top_level_window_id)
        self.resize_embedded_rviz()
        QTimer.singleShot(100, self.resize_embedded_rviz)
        QTimer.singleShot(500, self.resize_embedded_rviz)
    ...
    def _target_size(self) -> Tuple[int, int]:
        ...
        if parent is not None:
            width = max(width, int(parent.contentsRect().width()))
            height = max(height, int(parent.contentsRect().height()))
        return width, height
    ...
    def _fail(self, message: str) -> None:
        self._log(message)
        self._set_placeholder("RViz embed failed")
        if self.placeholder_label is not None:
