# gp7_task_executor_msgs Tasks

## Purpose

This task file tracks work specific to the `gp7_task_executor_msgs` package. This package defines all service types consumed by `gp7_task_executor` and its clients (`gp7_gui`, `gp7_drl_inference`, `gp7_vision_gazebo_bridge`). Service definitions must be stable and consistent across all packages.
...
- [ ] **TODO: verify** — Service field types match the C++ server implementation in `gp7_task_executor`: e.g., `MoveToCartesianTarget` has `x`, `y`, `z` (double), `frame_id` (string), `execute` (bool), and response has `success` (bool), `fraction` (double), `message` (string).
...
- [ ] **TODO: verify** — All clients (`gp7_gui`, `gp7_drl_inference`, `gp7_vision_gazebo_bridge`) import the generated service types correctly.

## Medium Priority

- [ ] **TODO: verify** — The `package.xml` correctly declares `rosidl_interface_packages` member group for automatic type discovery.
- [ ] **TODO: verify** — The package builds with `colcon build --packages-select gp7_task_executor_msgs` without errors.
