...
from gp7_drl_inference.drl_planner_core import (
    DrlTrajectoryPlannerCore,
    PlanningResult,
    load_planner,
)
from gp7_drl_inference.drl_planner_node_base import DrlPlannerNodeBase
from std_srvs.srv import Trigger

# Lazy import — the Box message type is optional.
try:
    from gp7_vision_pipeline.msg import Box  # noqa: F401
except ImportError:
...
def read_user_input() -> tuple[np.ndarray, bool, np.ndarray, np.ndarray]:
    ...
    tx = read_float("Target X (base_link m)", 0.030)
    ty = read_float("Target Y (base_link m)", -0.400)
    tz = read_float("Target Z (base_link m)", 0.110)
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        help="Model filename in install/share/gp7_drl_inference/models/"
    )
    parser.add_argument(
