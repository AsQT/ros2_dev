...
Deep Reinforcement Learning training and evaluation framework for the **Yaskawa GP7** 6-DOF industrial robotic arm, using **PyBullet** for physics simulation, **Gymnasium** for the RL environment API, and **Stable-Baselines3** for DRL algorithm implementations.
...
PyBullet Physics Engine (physics + OpenGL rendering)
         │
         ▼
  GP7ReachPyBulletEnv  (Gymnasium API)
         │
         ▼
...
│   │   └── Environment/
│   │       └── GP7ReachPyBulletEnv.py   # Main Gym env (PyBullet-backed)
│   ├── PyBullet/
...
├── Training/
│   ├── train_ddpg_gp7.py                 # DDPG, Default, 500k steps
...
## Robot Specifications — Yaskawa GP7
...
## Environment Design — GP7ReachPyBulletEnv

### Environment ID Format

```
YaskawaGP7ReachPyBullet-{Mode}-v0
```
...
Workspace bounds are derived from the "Search" configuration space vertices of the Yaskawa GP7. The bounds define the axis-aligned box within which the TCP is allowed to move.

---

## Training Configuration

### Hyperparameters — DDPG (train_ddpg_gp7.py)
...
|--------|------|-------|-----------|
| `train_ddpg_gp7.py` | Default | 500,000 | DDPG |
...
Some URDF files reference mesh paths that differ between Windows and Linux. The `Evaluation/URDFs/Robots/YASKAWA_GP7/a.py` script resolves these automatically.
