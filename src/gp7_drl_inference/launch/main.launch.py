...
Orchestrates all layers in the correct startup order:
  1. Robot bringup  (sim_mode via gp7_bringup)
  2. Vision         (mock via mock_environment_node, or real via gp7_vision_pipeline)
...
Usage:
  ...
  ros2 launch gp7_drl_inference main.launch.py vision:=mock
...
Arguments:
  ...
  vision                   — "mock" (synthetic data) or "real" (gp7_vision_pipeline)
...
def generate_launch_description() -> LaunchDescription:
    ...
    pkg_drl = "gp7_drl_inference"
    pkg_vision = "gp7_vision_pipeline"
    ...
    vision_arg = DeclareLaunchArgument(
        "vision",
        default_value="mock",
        description="'mock' (synthetic data) or 'real' (gp7_vision_pipeline)",
    )
    ...
    # ── Step 1: Robot bringup (sim_mode) ──────────────────────────────────────
    sim_bringup = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            [share_bringup, "/launch/sim_mode.launch.py"]
        ),
        launch_arguments={
            "use_rviz": "true",
            "start_task_executor": "true",
            "use_sim_time": use_sim_time,
            "start_scene_visualization": LaunchConfiguration("start_scene_visualization"),
    ...
    return LaunchDescription([
        LogInfo(msg="[main] GP7 DRL unified planner stack"),
        ...
        LogInfo(msg="[main] Step 1: Robot bringup (sim_mode)..."),
        sim_bringup,
