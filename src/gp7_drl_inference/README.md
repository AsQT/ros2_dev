...
ros2 launch gp7_drl_inference main.launch.py
...
ros2 run gp7_drl_inference drl_unified_planner_node \
    --ros-args \
    -p input_mode:=manual \
...
| `calibrated_start_tcp_base` | `[0.350, -0.330, 0.060]` | Start TCP in base_link frame [x, y, z] m |
...
Primary RViz topic: `/scene/markers` (via `gp7_visualization/scene_visualization_node`)

Debug source topics:
- `/drl/forward_trajectory_marker`
- `/drl/backward_trajectory_marker`

See `gp7_visualization/README.md` for details.
...
```bash
ros2 launch gp7_drl_inference main.launch.py
ros2 launch gp7_drl_inference main.launch.py input_mode:=vision
ros2 launch gp7_drl_inference main.launch.py vision:=real
```
...
```bash
ros2 launch gp7_drl_inference drl_unified_planner.launch.py
ros2 launch gp7_drl_inference drl_unified_planner.launch.py input_mode:=vision
```

### `mock_environment.launch.py` — Synthetic vision only

```bash
ros2 launch gp7_drl_inference mock_environment.launch.py
ros2 launch gp7_drl_inference mock_environment.launch.py target_x:=0.2 target_y:=-0.3 target_z:=0.4
```
...
3. Verify `gp7_visualization` is running: `ros2 node list | grep scene`
...
```bash
ls ~/pap_yaskawa_ws/src/gp7_drl_inference/models/
```
