# gp7_drl_inference Tasks

## Purpose

This task file tracks work specific to the `gp7_drl_inference` package. The package provides the `drl_unified_planner_node` which handles all DRL inference (DDPG/SAC/TD3 policy), planning, and execution for the GP7 pick-and-place workflow.
...
- If the unified planner publishes no markers: verify TF is available (`base_link → tool0`) and the vision topic is publishing.
