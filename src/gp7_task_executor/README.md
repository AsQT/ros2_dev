...
**Purpose:** High-level task executor for the Yaskawa GP7 using MoveIt 2 (C++). Provides a service-based interface to MoveIt, abstracting away trajectory planning and execution.
...
│   ├── pose_waypoints.yaml           # Named pose targets (meters + radians)
│   └── cartesian_points.yaml          # Named Cartesian points (meters only)
├── launch/
...
|------|-----------|---------|
| `task_executor_node` | `task_executor_node` | C++ service node — exposes all task services |
...
pose_waypoints:
  pose_A:
    ...
    rpy: [3.1416, 0.0, 0.0]
  pose_B:
    position: [0.00254, -0.4966, 0.1639]
    rpy: [-2.18, 1.18, -0.02]
```
...
All services use types from `gp7_task_executor_msgs`.
...
| `/move_sequence` | `MoveSequence` | Execute a sequence of named joint waypoints |
| `/move_cartesian_sequence` | `MoveCartesianSequence` | Execute a sequence of named Cartesian points |
...
ros2 service call /move_cartesian_pose_sequence \
  gp7_task_executor_msgs/srv/MoveCartesianPoseSequence \
...
- [ ] Verify joint_waypoint names match the YAML config keys (e.g., `start`, `origin`, `P1`)
- [ ] Verify the cartesian_points RPY orientation is correct for the gripper/tool being used
