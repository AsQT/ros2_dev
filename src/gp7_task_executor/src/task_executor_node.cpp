...
#include "gp7_task_executor_msgs/srv/move_cartesian_pose_sequence.hpp"
...
private:
  ...
  void create_services()
  {
    ...
    service_cartesian_pose_sequence_ = create_service<gp7_task_executor_msgs::srv::MoveCartesianPoseSequence>(
        "/move_cartesian_pose_sequence",
        [this](
            const std::shared_ptr<gp7_task_executor_msgs::srv::MoveCartesianPoseSequence::Request>& request,
            const std::shared_ptr<gp7_task_executor_msgs::srv::MoveCartesianPoseSequence::Response>& response)
      ...
      const std::shared_ptr<gp7_task_executor_msgs::srv::MoveCartesianPoseSequence::Response>& response)
  {
    ...
    // Fallback orientation: Cartesian tool orientation RPY=[pi,0,0].
    ...
    // Log current tool0 pose from MoveIt (if available)
    try
    {
      const auto current_eef = move_group_->getCurrentPose().pose;
      RCLCPP_INFO(get_logger(),
                  "[%s] Current tool0 xyz=(%.4f, %.4f, %.4f)  "
                  "quat=(%.6f, %.6f, %.6f, %.6f)",
                  "/move_cartesian_pose_sequence",
    ...
    {
      RCLCPP_WARN(get_logger(), "[%s] Could not get current tool0 pose: %s",
    ...
    for (size_t i = 0; i < poses.size(); ++i)
    {
      ...
      if (q_norm < 1e-9)
      {
        RCLCPP_WARN(get_logger(),
                    ...
                    "fallback quat=(%.6f, %.6f, %.6f, %.6f)  (RPY=[pi, 0, 0])",
    ...
    RCLCPP_INFO(get_logger(),
                ...
                gp7_task_executor::PlannerUtils::CARTESIAN_SUCCESS_THRESHOLD,
                result.success);
  ...
  rclcpp::Service<gp7_task_executor_msgs::srv::MoveCartesianPoseSequence>::SharedPtr service_cartesian_pose_sequence_;
