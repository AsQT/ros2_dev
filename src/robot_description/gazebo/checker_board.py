...
class FixedOnTableSpawner(Node):
    def __init__(self):
        ...
        # Chiều cao khối gỗ (m).
        self.declare_parameter("checker_board_height", 0.05)

        # Nhích lên 1 chút để tránh cấn mặt bàn
        self.declare_parameter("z_epsilon", 0.005)

        pkg_share = get_package_share_directory("robot_description")
        self.sdf_path = os.path.join(pkg_share, "worlds", "checker_board", "checker_board.sdf")
        ...
        self.spawn_checker_board()

    def spawn_checker_board(self):

        table_z      = float(self.get_parameter("table_z").value)
        block_h      = float(self.get_parameter("checker_board_height").value)
        ...
        # (name, x, y, yaw_deg)
        targets = [
            ("checker_board", 0.50, 0.0, 0.0),
        ]
        self.get_logger().info("--- Spawn checker_board MAT BAN  ---")
        self.get_logger().info(f"SDF: {self.sdf_path}")
        self.get_logger().info(f"table_z={table_z:.3f}, checker_board_height={block_h:.3f} => z_on_table={z_on_table:.3f}")
