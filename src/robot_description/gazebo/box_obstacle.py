#!/usr/bin/env python3
import os
import math
import subprocess
import time

import rclpy
from rclpy.node import Node
from ament_index_python.packages import get_package_share_directory


class BoxObstacleSpawner(Node):
    def __init__(self):
        super().__init__("box_obstacle_spawner")

        # --- Params ---
        self.declare_parameter("startup_delay", 2.5)

        # Độ cao mặt bàn trong world
        self.declare_parameter("table_z", 1.10)

        # Chiều cao box trắng trong file worlds/box/box_model.sdf
        self.declare_parameter("box_height", 0.08)

        # Nhích lên 1 chút để tránh cấn mặt bàn
        self.declare_parameter("z_epsilon", 0.005)

        # Vị trí box trên bàn
        self.declare_parameter("x", 0.50)
        self.declare_parameter("y", 0.18)
        self.declare_parameter("yaw_deg", 0.0)

        pkg_share = get_package_share_directory("robot_description")
        self.sdf_path = os.path.join(pkg_share, "worlds", "box", "box_model.sdf")

        if not os.path.exists(self.sdf_path):
            self.get_logger().error(f"LOI: Khong tim thay file SDF: {self.sdf_path}")
            return

        time.sleep(max(0.0, float(self.get_parameter("startup_delay").value)))
        self.spawn_box_obstacle()

    def spawn_box_obstacle(self):
        table_z = float(self.get_parameter("table_z").value)
        box_h   = float(self.get_parameter("box_height").value)
        z_eps   = float(self.get_parameter("z_epsilon").value)

        x       = float(self.get_parameter("x").value)
        y       = float(self.get_parameter("y").value)
        yaw_deg = float(self.get_parameter("yaw_deg").value)

        z_on_table = table_z + box_h * 0.5 + z_eps

        roll  = 0.0
        pitch = 0.0
        yaw   = math.radians(yaw_deg)

        self.get_logger().info("--- Spawn box trang tren MAT BAN ---")
        self.get_logger().info(f"SDF: {self.sdf_path}")
        self.get_logger().info(f"table_z={table_z:.3f}, box_height={box_h:.3f} => z_on_table={z_on_table:.3f}")

        self.spawn_via_command("box_obstacle", x, y, z_on_table, roll, pitch, yaw)

    def spawn_via_command(self, name, x, y, z, roll=0.0, pitch=0.0, yaw=0.0):
        cmd = [
            "ros2", "run", "ros_gz_sim", "create",
            "-name", name,
            "-x", str(x), "-y", str(y), "-z", str(z),
            "-R", str(roll), "-P", str(pitch), "-Y", str(yaw),
            "-file", self.sdf_path,
            "-allow_renaming", "true",
        ]

        self.get_logger().info(
            f"Spawn {name} @ (x={x:.3f}, y={y:.3f}, z={z:.3f}) | RPY=({roll:.2f},{pitch:.2f},{yaw:.2f})"
        )

        try:
            result = subprocess.run(cmd, capture_output=True, text=True)
            if result.returncode == 0:
                self.get_logger().info(f"-> OK: {name}")
            else:
                self.get_logger().error(f"-> LOI spawn {name}: {result.stderr.strip()}")
        except Exception as e:
            self.get_logger().error(f"Exception spawn {name}: {e}")


def main():
    rclpy.init()
    node = BoxObstacleSpawner()
    node.destroy_node()
    rclpy.shutdown()


if __name__ == "__main__":
    main()
